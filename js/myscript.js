...
for (let i = 0; i < membriTeam.length; i++) {
    ...
    // elementi in dom con template litteral
    ...
    // oppure elementi in dom con il createElement:
    const li = document.createElement('li');
    li.classList.add('card');

    const img = document.createElement('img');
    img.src = `img/${membriSignoli.photo}`;
    img.alt = membriSignoli.photo;
    img.classList.add('image-set')
    li.append(img);
    
    const h3Elemento = document.createElement('h3');

    //gestire caso se non c'è il nome TO DOOOO
    if (membriSignoli.nome !== undefined){
        h3Elemento.textContent = membriSignoli.nome;
    } else {
